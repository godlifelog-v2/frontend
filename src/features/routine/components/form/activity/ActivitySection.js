...
function ActivitiesSection({
  ...
  const addActivity = () => {
    ...
    append({ activityIdx: 0, activityName: "", setTime: newStartTime, description: "", activityImp: 3, verified: false });
  };
  ...
  return (
    <div className="space-y-4">
      ...
      {formState.errors.activities && (
        <div className="text-red-500 text-sm mt-2">{formState.errors.activities.message}</div>
      )}
      ...
      {(routineData?.isWriter === 1 || routineData == null) && !readOnly &&
        fields.map((field, index) => (
          <Card key={field.id} className="p-4 relative">
            <FormField
              ...
              render={({ field: fieldProps }) => <input type="hidden" {...fieldProps} value={fieldProps.value ?? 0} />}
            />
            ...
            <div className="flex justify-between items-center mb-2">
              ...
              {!readOnly && (
                <Button type="button" variant="ghost" size="sm" className="text-red-500 hover:text-red-700 hover:bg-red-50 p-2" onClick={() => remove(index)}>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="flex-1">
                    ...
                    <FormControl><Input placeholder="예: 아침 운동, 독서 등" {...field} disabled={readOnly} readOnly={readOnly} /></FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              />
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl><Input type="time" placeholder="HH:MM" {...field} disabled={readOnly} readOnly={readOnly} /></FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              />
                ...
                render={() => (
                  <FormItem>
                    <FormLabel className="flex items-center gap-1">
                      <Star className="h-3.5 w-3.5 fill-yellow-400 text-yellow-400" />중요도
                    </FormLabel>
                    <FormControl>
                      <div className="pt-1">
                        <StarRating control={control} name={`activities.${index}.activityImp`} maxRating={5} required={true} readOnly={readOnly} />
                      </div>
                    </FormControl>
                    <FormMessage />
                  </FormItem>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel className="flex items-center gap-1"><FileText className="h-3.5 w-3.5" />한줄 메모</FormLabel>
                  <FormControl><Input placeholder="활동에 대한 간단한 메모를 남겨보세요" {...field} disabled={readOnly} readOnly={readOnly} /></FormControl>
                  <FormMessage />
                </FormItem>
              )}
            />
